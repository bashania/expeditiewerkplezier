...
const SCAN_REVIEWS = [
{ q: "Ik dacht echt dat het aan mij lag. Maar door de uitleg viel alles op zijn plek. Dat gaf zoveel rust in mijn hoofd.", name: "Marleen K." },
{ q: "Ik functioneerde nog wel, maar alles kostte me zoveel energie. De scan liet me zien wat er écht speelde.", name: "Joëlle V." },
{ q: "Door dit inzicht kon ik eindelijk stoppen met mezelf pushen.", name: "Sanne T." }];
...
function ScanOptin({ compact, onCheckout }) {
  return (
    <div className={"ewk-scanform" + (compact ? " ewk-scanform--compact" : "")}>
      ...
      {!compact && <p className="ewk-scanform__note">Je wordt doorgestuurd naar een korte, beveiligde aanmelding. Je kunt je altijd weer afmelden.</p>}
    </div>);
...
function ScanCtaBand({ onCheckout, eyebrow, title, sub, label }) {
  return (
    <section className="ewk-ctaband">
      <div className="ewk-wrap ewk-ctaband__inner">
        {eyebrow && <Eyebrow>{eyebrow}</Eyebrow>}
        <h2 className="ewk-h2">{title}</h2>
        {sub && <p>{sub}</p>}
        <Button variant="primary" size="lg" iconRight="arrow-right" onClick={onCheckout}>{label || "Doe de gratis scan"}</Button>
      </div>
    </section>);
    ...
    <main>
        ...
        <div className="ewk-wrap ewk-hero__grid ewk-scan-hero">
          <div>
            ...
            <p className="ewk-hero__lead">
              Je houdt alles draaiende. Je hoofd staat nooit stil en stoppen lukt niet meer. Ontdek in 
              <span className="ewk-key">10 minuten</span> wat
      ...
      <section className="ewk-section">
        <div className="ewk-wrap ewk-prose" style={{ margin: "0 auto", textAlign: "center" }}>
          ...
          <h2 className="ewk-h2" style={{ margin: "12px 0 24px" }}>Het rare is: Je functioneert nog, maar het kost je steeds meer energie.</h2>
          <p>
          ...
          <p>
            ...
            De scan laat je zien wat er onder die vermoeidheid, onrust en prikkelbaar ligt. Niet als standaard test, maar als concreet inzicht wat er in jouw brein en lichaam speelt. Zodat je stopt met twijfelen aan
            jezelf en exact weet wat je nodig hebt.
          </p>
        </div>
      </section>
        ...
        <div className="ewk-wrap">
          ...
          <div className="ewk-testgrid">
            ...
            <figure className="ewk-testcard" key={i}>
                ...
                <figcaption><span className="ewk-testcard__tag"><Icon name="quote" />{r.name}</span></figcaption>
              </figure>
            )}
          </div>
        </div>
      ...
      {/* CTA-band na de reviews */}
      <ScanCtaBand onCheckout={onCheckout}
      ...
      <section className="ewk-finalcta ewk-finalcta--scan">
        <div className="ewk-wrap ewk-scan-close">
          <div className="ewk-scan-close__text">
            ...
            <p>Als jij ergens voelt dat dit over jou gaat, dan is dit misschien precies wat je nu nodig hebt. Je probeert het al lang genoeg alleen.

Soms is het gewoon fijn om even uit je eigen denk-cirkel te komen en met een frisse blik naar jouw uitdagingen te kijken. Hierdoor heb je direct helder wat je te doen staat.
            </p>
          </div>
...
Object.assign(window, { ScanPage, ScanOptin, ScanCtaBand });